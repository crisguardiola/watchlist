<script lang="ts">
	...
	let favoriteMovies = $state<TmdbResult[]>([]);
	...
	async function fetchProfileData() {
		loading = true;
		try {
			const res = await fetch('/api/profile-data', { credentials: 'include' });
			if (!res.ok) return;
			const data = await res.json();
			genres = data.genres ?? [];
			favoriteMovies = data.favoriteMovies ?? [];
			genreIds = (data.preferences?.genreIds ?? []) as number[];
...
{#if open}
	...
	>
		<div
			class="search-popover preferences-popover"
			role="dialog"
			...
			onkeydown={(e) => e.key === 'Escape' && onClose()}
		>
			<div class="preferences-popover-content">
				<h2 class="preferences-popover-title">Preferences</h2>
					...
					<form
						...
						use:enhance={() => {
							return async ({ result }) => {
								...
								// Don't call update() - form posts to /profile, we stay on current page
							};
						}}
